...
def preprocess_image(image_path: str, target_height: int = 64) -> np.ndarray | None:
    ...
    Returns:
        Предобработанное изображение NumPy (float32) или None, если загрузка не удалась.
    """
    ...
    # Символы становятся белыми (255), фон - черным (0)
    ...
    # Используем cv2.INTER_AREA для уменьшения, cv2.INTER_LINEAR для увеличения
    ...
    # Модели часто ожидают канал "цвета", даже если он один
...
#         # Показать результат (нужно убрать expand_dims или squeeze для imshow)
